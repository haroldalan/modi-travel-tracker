...
// Initialize Globe with arrow settings
function initGlobe() {
  const globe = Globe()
    ...
    .arcPointerResolution(12)
    .arcPointerEdgeLength(0.2)
    .arcPointerAngle(Math.PI/4)
...
function createArcs(locations) {
  return locations.slice(0, -1).map((a, i) => ({
    ...
    color: ['rgba(255, 102, 0, 0.8)', 'rgba(255, 102, 0, 0.4)'],
    stroke: 0.5,
    arcSharpness: 0.85,
    pointer: true,
    pointerWidth: 0.5,
...
async function updateGlobe() {
  ...
  globe
    ...
    .arcPointer('pointer')
    .arcPointerWidth('pointerWidth')
    .arcPointerLength('pointerLength')
