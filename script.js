...
// State\let currentDate = new Date();
let travelData = {}; // cache
let availableMonths = [];

// 1) Init globe without backgroundImageUrl
const globe = Globe()
...
// Make scene transparent to show stars behind
...
// Navigation
prevMonthBtn.addEventListener('click', () => { const idx = availableMonths.indexOf(currentDate.toLocaleDateString('en-US', DATE_FORMAT)); if (idx>0) currentDate = new Date(availableMonths[idx-1]), updateGlobe(); });
nextMonthBtn.addEventListener('click', () => { const idx = availableMonths.indexOf(currentDate.toLocaleDateString('en-US', DATE_FORMAT)); if (idx>=0 && idx<availableMonths.length-1) currentDate = new Date(availableMonths[idx+1]), updateGlobe(); });
